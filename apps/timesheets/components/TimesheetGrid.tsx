...
export function TimesheetGrid({ employee, monday, onApprovalChange }: Props) {
  ...
  useEffect(() => {
    (async () => {
      let all: { projectnumber: string; item_seq: number; line_desc: string; completed_at: string | null }[] = [];
      let from = 0;
      const pageSize = 1000;
      while (true) {
        const { data, error } = await supabase
          .from("project_register_items")
          .select("projectnumber, item_seq, line_desc, completed_at")
          .order("projectnumber")
          .order("item_seq")
          .range(from, from + pageSize - 1);
        if (error) {
          console.error("[timesheet] project_register_items load failed:", error);
          break;
        }
